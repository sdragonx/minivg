...
#define MINIVG_CLASS_NAME   PCWSTR(L"minivg_window")   // 窗口类名
#define MINIVG_DEFAULT_FONT PCWSTR(L"Microsoft YaHei") // 微软雅黑
...
{
private:
    std::map<unistring, vgImage*> images;    // 加载的图片
    std::map<int, vgImage*> resource_images; // 加载的资源图片
    std::vector<vgImage*> image_pool;        // 创建的图片
...
initgraph(const unistring& title, int width, int height, int param)
{
    ...
    // std::locale::global(std::locale("zh_CN"));    // c++ 中文环境
...
{
    // 得到格式为 format 的图像文件的编码值，访问该格式图像的 COM 组件的 GUID 值保存在 pCLSID 中
    UINT num  = 0;
    UINT size = 0;
...
MINIVG_INLINE void drawsprite(
    ...
    float rotation,                        // 旋转角度
    float centerX, float centerY,          // 旋转中心(0.0 - 1.0 之间，按源大小比例设置旋转中心)
    float alpha
)
{
    (void) alpha; // todo... 图片 alpha 支持
    ...
    if (g && image) {
        ...
        Gdiplus::Matrix m;
        m.Translate(x, y);       // 平移
        m.Rotate(-rotation);     // 旋转
        m.Scale(scaleX, scaleY); // 缩放
        g->SetTransform(&m);     // 应用矩阵变换
