...
// 判断是否包含点
template<typename T>
bool contains(const vec4<T>& rect, T vx, T vy)
{
    return vx >= rect.x && vx <= (rect.x + rect.width) && vy >= rect.y && vy <= (rect.y + rect.height);
}
...
vec4<T> operator&(const vec4<T>& a, const vec4<T>& b)
{
    ...
    return vec4<T>(
        max_x,
        max_y,
        min(a.x + a.width, b.x + b.width) - max_x,
        min(a.y + a.height, b.y + b.height) - max_y
    );
}
